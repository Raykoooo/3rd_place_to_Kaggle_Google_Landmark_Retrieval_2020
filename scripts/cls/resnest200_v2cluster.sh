...
#--------------------prepare data-------------------#
SRC0_DATA_DIR="data"
SRC0_LABEL_PATH="${SRC0_DATA_DIR}/data_list/label_sub_212843.txt"

TAG="resnest200_v2cluster_448_GPU"${1}
BACKBONE="resnest200"
MODEL_NAME="cls_model"
CHECKPOINTS_NAME="google_landmark_2020_"$TAG
CHECKPOINTS_DIR="checkpoints/cls/${BACKBONE}"

PRETRAINED="pretrained_models/resnest200-75117900.pth"
# RESUME_MODEL="${CHECKPOINTS_DIR}/${CHECKPOINTS_NAME}_latest.pth"
RESUME_MODEL="${CHECKPOINTS_DIR}/google_landmark_2020_resnest200_v2clean_448_GPU8_latest.pth"
...
${PYTHON} main.py --config_file ${CONFIG_FILE} --gpu ${GPU_LIST} --workers 4 --num_data_sources 1 \
                  --src0_data_dir ${SRC0_DATA_DIR} --src0_label_path ${SRC0_LABEL_PATH} --src0_train_batch_size ${TRAIN_BATCH_SIZE} --src0_val_batch_size ${VAL_BATCH_SIZE} --src0_num_classes 212843 --src0_min_count 5 --src0_linear_type arc0.3_30  --src0_train_loader default \
                  --backbone ${BACKBONE} --model_name ${MODEL_NAME} --loss_type ${LOSS_TYPE} --fc_dim 512 --fc_bn y --fc_relu n \
                  --max_iters ${MAX_ITERS} --base_lr ${BASE_LR} --bb_lr_scale 0.1 --is_warm y --warm_iters 20000 --warm_freeze y --test_interval ${TEST_INTERVAL} --dist y --gather n --display_iter 100 \
