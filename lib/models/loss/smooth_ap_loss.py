...
# origined from https://github.com/Andrew-Brown1/Smooth_AP.git
...
class SmoothAPLoss(nn.Module):
    def __init__(self, configer):
        ...
        if 'smooth_ap_loss' in configer.get('loss', 'params'):
            self.params_dict = configer.get('loss', 'params')['smooth_ap_loss']

        self.anneal = self.params_dict['anneal']
        self.batch_size = self.params_dict['batch_size']
        self.num_id = self.params_dict['num_id']
        self.feat_dims = self.params_dict['feat_dims']

    def forward(self, inputs, targets):
        """Forward pass for all input predictions: inputs - (batch_size x feat_dims) """
        ...
        xs = inputs.view(self.num_id, int(self.batch_size / self.num_id), self.feat_dims)
