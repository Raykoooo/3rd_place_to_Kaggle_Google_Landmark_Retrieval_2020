...
from lib.models.loss.sigma import functional as F
from lib.models.loss.sigma.utils import (detect_large, split_1, split_2,
                                         split_KN_pos)


class PrecisionAtTopkLoss(nn.Module):
    def __init__(self, configer):
        super(PrecisionAtTopkLoss, self).__init__()
        self.params_dict = dict()
        if 'precision_at_topk_loss' in configer.get('loss', 'params'):
            self.params_dict = configer.get('loss', 'params')['precision_at_topk_loss']
        self.sparsity = 0
        self.err_pos = 0
        self.tau = self.params_dict['tau']
        self.margin = self.params_dict['margin']
        self.thresh = self.params_dict['thresh']
    ...
    def N_posBiggerThanK(self,x, y_,k):
        x_1, x_2 = split_1(x, y_)# add margin x1 all score ,x2 n_pos score
