...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, deep_base=False, norm_type=None, rm_last_stride=False, rm_last_bottleneck=False):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, norm_type=norm_type, rm_last_stride=rm_last_stride)
        ...
        for m in self.modules():
            ...
            elif isinstance(m, ModuleHelper.BatchNorm2d(norm_type=norm_type)):
                m.weight.data.fill_(1)
                m.bias.data.zero_()

    def _make_layer(self, block, planes, blocks, stride=1, norm_type=None, rm_last_stride=False):
        downsample = None
